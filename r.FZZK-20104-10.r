...
data_use <- data_use[!is.na(data_use$Gene_symbol)&!grepl('///',data_use$Gene_symbol),]
data_use <- data_use[data_use$Gene_symbol!="",]
data_use<- as.data.frame(avereps(data_use[,-41],ID = data_use$Gene_symbol) )
...
data_use <- data_use[!is.na(data_use$Gene_symbol)&!grepl('///',data_use$Gene_symbol),]
data_use <- data_use[data_use$Gene_symbol!="",]
data_use<- as.data.frame(avereps(data_use[,-81],ID = data_use$Gene_symbol) )
data_use <- na.omit(data_use)#5932
View(data_use)
...
eset_dat <- hmexp
index <- DEG_id1
index_matrix <- t(scale(t(eset_dat[index,])))
...
Gene = bitr(rownames(DEG_18),fromType = "SYMBOL", toType = "ENTREZID", OrgDb = "org.Hs.eg.db") 
Gene <- na.omit(Gene) 
#FClist <- t(DiffGene)  
#FClist <- FClist[1,]
...
#data[,1] <- as.factor(data[,1])#因子是group
...
kable(acc, "html", caption = "<center>**表. SVM(Training set) **</center>") %>%
  kable_styling(bootstrap_options = "striped", full_width = F)
...
##############
rm(list = ls())
expr <- fread("E:/FZZK-20104-10/00_rawdata/GSE13850/03_expr.csv")
expr <- as.data.frame(expr)
rownames(expr) <- expr$V1
expr <- expr[,-1]
#expr <- t(expr)
...
data3$points<-points_cal(formula = results$formula,rd=data3)
pre<-data3$points

pdf("roc.pdf",height = 10,width = 8)
...
########MCL1
setwd("E:/FZZK-20104-10/09_GSEA/kegg")
rm(list = ls())
...
head(gsea)[1:10]
write.csv(as.data.frame(gsea),"02_GSEA_MCL1.csv",row.names = F)

geneSetID = c('KEGG_SPLICEOSOME',
              'KEGG_NEUROACTIVE_LIGAND_RECEPTOR_INTERACTION',
              'KEGG_NOD_LIKE_RECEPTOR_SIGNALING_PATHWAY',
              'KEGG_UBIQUITIN_MEDIATED_PROTEOLYSIS',
              'KEGG_CALCIUM_SIGNALING_PATHWAY')
# all plot
pdf(file ="03_GSEA_kegg_MCL1.pdf",width=10,height=8,onefile=FALSE )
gseaNb(object = gsea,
...
#plan(multiprocess)
#nbrOfWorkers()
system.time(dd <- batch_cor("TRAP1")) 
gene <- dd$Symbol
...
gsea <- setReadable(gsea, 'org.Hs.eg.db', 'ENTREZID')
head(gsea)[1:10]
write.csv(as.data.frame(gsea),"04_GSEA_TRAP1.csv",row.names = F)
...
# all plot
pdf(file ="05_GSEA_kegg_TRAP1.pdf",width=12,height=8,onefile=FALSE )
gseaNb(object = gsea,
       geneSetID = geneSetID,termWidth = 35,
       curveCol = brewer.pal(5,'Paired'))
dev.off()
...
#plan(multiprocess)
#nbrOfWorkers()
system.time(dd <- batch_cor("PMAIP1")) 
gene <- dd$Symbol
...
gsea <- setReadable(gsea, 'org.Hs.eg.db', 'ENTREZID')
head(gsea)[1:10]
write.csv(as.data.frame(gsea),"06_GSEA_PMAIP1.csv",row.names = F)

geneSetID = c('KEGG_REGULATION_OF_ACTIN_CYTOSKELETON',
              'KEGG_FOCAL_ADHESION',
              'KEGG_FC_GAMMA_R_MEDIATED_PHAGOCYTOSIS',
              'KEGG_PATHWAYS_IN_CANCER',
              'KEGG_CHRONIC_MYELOID_LEUKEMIA')
# all plot
pdf(file ="07_GSEA_kegg_PMAIP1.pdf",width=12,height=8,onefile=FALSE )
gseaNb(object = gsea,
       geneSetID = geneSetID,termWidth = 35,
       curveCol = brewer.pal(5,'Paired'))
dev.off()
...
########MCL1
setwd("E:/FZZK-20104-10/09_GSEA/go")
rm(list = ls())
...
go_gmt <- read.gmt("01_c5.go.bp.v2023.2.Hs.symbols.gmt") 
gsea <- GSEA(geneList,
             TERM2GENE = go_gmt) 
#gsea <- setReadable(gsea, 'org.Hs.eg.db', 'ENTREZID')
#head(gsea)[1:10]
...
geneSetID = c('GOBP_NUCLEAR_TRANSPORT',
...
# all plot
pdf(file ="03_GSEA_go_MCL1.pdf",width=10,height=8,onefile=FALSE )
gseaNb(object = gsea,
       geneSetID = geneSetID,termWidth = 35,
       curveCol = brewer.pal(5,'Paired'))
dev.off()
...
geneSetID = c('GOBP_SODIUM_ION_TRANSPORT',
...
# all plot
pdf(file ="05_GSEA_go_TRAP1.pdf",width=10,height=8,onefile=FALSE )
gseaNb(object = gsea,
       geneSetID = geneSetID,termWidth = 35,
       curveCol = brewer.pal(5,'Paired'))
dev.off()
...
geneSetID = c('GOBP_RIBONUCLEOPROTEIN_COMPLEX_BIOGENESIS',
              'GOBP_CYTOPLASMIC_TRANSLATION',
              'GOBP_TRANSLATIONAL_INITIATION',
              'GOBP_RNA_SPLICING_VIA_TRANSESTERIFICATION_REACTIONS',
              'GOBP_RIBOSOME_BIOGENESIS')
# all plot
pdf(file ="07_GSEA_go_PMAIP1.pdf",width=10,height=8,onefile=FALSE )
gseaNb(object = gsea,
       geneSetID = geneSetID,termWidth = 35,
       curveCol = brewer.pal(5,'Paired'))
dev.off()
